...
class StochasticWindyGridworld:
    ...
    def __init__(self, initialize_model=True):
        ...
        self.goal_rewards = [
            100]  # a vector specifying the associated rewards with the goals in self.goal_locations, in [r1,r2] format
    ...
    def step(self, a):
        ...
        if goal_present:
            goal_index = \
            np.where([np.all(goal_location == self.agent_location) for goal_location in self.goal_locations])[0][0]
            done = True
    ...
    def _construct_model(self):
        ...
        r_sas = np.zeros((self.n_states, self.n_actions,
                          self.n_states)) + self.reward_per_step  # set all rewards to the default value
        ...
        for s in range(self.n_states):
            for a in range(self.n_actions):
                ...
                if state_is_a_goal:
                    ...
                    next_location_with_wind = np.clip(next_location_with_wind, (0, 0),
                                                      np.array(self.shape) - 1)  # bound within grid
                    ...
                    next_location_without_wind = np.clip(next_location_without_wind, (0, 0),
                                                         np.array(self.shape) - 1)  # bound within grid
    ...
    def _initialize_plot(self):
        ...
        for i in range(len(self.goal_locations)):
            ...
            self.ax.text(self.goal_locations[i][0] + 0.05, self.goal_locations[i][1] + 0.75, text, fontsize=20,
                         c=colour)
...
def test():
    ...
    for t in range(n_test_steps):
        ...
        print(
            "State {}, Action {}, Reward {}, Next state {}, Done {}, p(s'|s,a) {}, r(s,a,s') {}".format(s, a, r, s_next,
                                                                                                        done, p_sas,
                                                                                                        r_sas))
